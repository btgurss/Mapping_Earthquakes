...
// Creating the tile layer that will be the background of map.
...
// Creating the second tile layer that will be the background of map.
...
//Second layer for tectonic plates
let tectonics = new L.LayerGroup();

//Third layer for major earthquakes
let majors = new L.LayerGroup();

// Add a reference to the tectonic plates group to the overlays object.
let overlays = {
  "Earthquakes": allEarthquakes,
  "Tectonic Plates": tectonics,
  "Major Earthquakes" : majors
};
...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson").then(function(data) {
  ...
  function styleInfo(feature) {
    return {
      ...
      // Sends magnitude to function to determine color of circle
      fillColor: getColor(feature.properties.mag),
      color: "#000000",

      // Sends magnitude to function to determine size of circle
...
//Styling the tectonic lines
function styleTectonics(feature) {
  return {
      //Making the color purple for easy to read
      color: "#de49d2",
      weight: 2
  };
}
  ...
  L.geoJson(data, {
    //Going through each feature and plotting the linestrings  
    onEachFeature: function(feature, layer) {
      layer.bindPopup();
    },
    //Calling function for tectonic place line style
    style: styleTectonics
  //Adding information to the tectonic overlay
  }).addTo(tectonics);
  //Adding the tectonic overlay to the map in the on position
  tectonics.addTo(map);
});
